...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text="Yo just share a banner link"
    )


def get_mission_url(bannergress_mission_id, mission_number):
    resp = requests.get(
        "https://api.bannergress.com/bnrs/" + str(bannergress_mission_id)
    )
    j = resp.json()
    ...
    return (
        "https://link.ingress.com/?link=https%3a%2f%2fintel.ingress.com%2fmission%2f"
        + active_mission_id
        + "&apn=com.nianticproject.ingress&isi=576505181&ibi=com.google.ingress&ifl=https%3a%2f%2fapps.apple.com%2fapp%2fingress%2fid576505181&ofl=https%3a%2f%2fintel.ingress.com%2fmission%2f"
        + active_mission_id
        + ""
    )
...
def parse_first_message(update: Update):
    ...
    if "https://bannergress.com/banner/" in message:
        ...
        bannergress_banner = requests.get(
            "https://api.bannergress.com/bnrs/" + bannergress_banner_id
        )
        if bannergress_banner.status_code is not 200:
            raise Exception(
                "It looks like you are not sharing a correct Bannergress URL. Please double-check your "
            ...
            return (
                bannergress_banner_json["title"],
                bannergress_banner_json["id"],
                bannergress_banner_json["numberOfMissions"],
    ...
    # The message is a link shared from the Ingress scanner
        ...
        resp = requests.get(
            "https://api.bannergress.com/bnrs?missionId=" + niantic_first_mission_id
        )
        if resp.status_code is not 200:
            ...
            return (
                bannergress_mission["title"],
                bannergress_mission["id"],
                bannergress_mission["numberOfMissions"],
        ...
        for entity in update.message.entities:
            if not entity or not entity["url"]:
                ...
                if "https://bannergress.com/banner/" in url:
                    bannergress_banner_id = url.replace(
                        "https://bannergress.com/banner/", ""
                    )
                    bannergress_banner = requests.get(
                        "https://api.bannergress.com/bnrs/" + bannergress_banner_id
                    )
                    if bannergress_banner.status_code is not 200:
                        ...
                        niantic_first_mission_id = bannergress_banner["missions"]["0"][
                            "id"
                        ]
                        return (
                            bannergress_banner["title"],
                            bannergress_banner["id"],
                            bannergress_banner["numberOfMissions"],
                            niantic_first_mission_id,
                        )

    raise Exception(
        "It looks like your message did not contain a Bannergress link or a mission link sent from the"
...
async def first_mission(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        (
            ...
            niantic_first_mission_id,
        ) = parse_first_message(update)
    except Exception as e:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text=fix_reserved_characters(str(e))
        )
        return
    ...
    reply_keyboard = [
        [
            InlineKeyboardButton(
                "Open in Scanner", url=get_mission_url(bannergress_id, "1")
            )
        ],
        [
            InlineKeyboardButton(
                "Next mission", callback_data=niantic_first_mission_id + ":::1"
            )
        ],
    ]
    await update.message.reply_text(
        fix_reserved_characters(title)
        + "\n\nYou are currently on mission: 1 of "
        + str(total_number_of_missions)
        + "\n\n",
    ...
    if resp.status_code is not 200:
        ...
        if int(mission_number) > int(total_number_of_missions):
            ...
            reply_keyboard = [
                [
                    InlineKeyboardButton(
                        "Open in Scanner",
                        url=get_mission_url(
                            mission_details["id"], str(int(mission_number))
                        ),
                    )
                ],
                [
                    InlineKeyboardButton(
                        "Previous mission",
                        callback_data=str(banner_id)
                        + ":::"
                        + str(int(mission_number) - 2),
                    )
                ],
                [
                    InlineKeyboardButton(
                        "Next mission",
                        callback_data=str(banner_id) + ":::" + mission_number,
                    )
                ],
            ]
            ...
            await query.edit_message_text(
                ...
                "You are currently on mission: "
                + mission_number
                + " of "
                + str(mission_details["numberOfMissions"])
                + "\n\n",
                reply_markup=InlineKeyboardMarkup(reply_keyboard),
                parse_mode=ParseMode.MARKDOWN_V2,
            )
...
if __name__ == "__main__":
    ...
    first_mission_handler = MessageHandler(
        filters.TEXT & (~filters.COMMAND), first_mission
    )
